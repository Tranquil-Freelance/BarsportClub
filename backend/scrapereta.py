...
def fetch_age_from_sofifa(player_name, session):
    ...
    try:
        ...
        if response.status_code == 429:
            ...
            time.sleep(1800)
            return fetch_age_from_sofifa(player_name, session)
...
def main():
    try:
        ...
        # Inizializziamo la Sessione persistente
        ...
        for idx, player in enumerate(players):
            ...
            age = fetch_age_from_sofifa(player, session)
            ...
            # Livello 1 Anti-ban: Jitter ULTRA-VELOCE (0.1 - 0.3 secondi)
            sleep_time = random.uniform(0.1, 0.3)
            logger.info(f"Jitter ultra-veloce: attesa di {sleep_time:.2f} secondi...")
