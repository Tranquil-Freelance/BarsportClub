"""
🚀 ESTRATTORE SERIE A - METODO "PARTITA PER PARTITA" (Clone Premier League)
...
# Log visibile in tempo reale nel terminale
logging.basicConfig(
    level=logging.INFO, 
...
# ─── SISTEMI DI SICUREZZA (Identici a ieri) ───────────────────────────
...
async def trigger_hibernation():
    logging.warning("🔴 [ALLARME BAN] Errore 429 rilevato. Ibernazione profonda (30 min) per resettare l'IP.")
    await asyncio.sleep(1800)

# ─── DATABASE (Tabella Giocatori PER PARTITA) ─────────────────────────
def initialize_database():
    query = text("""
        CREATE TABLE IF NOT EXISTS seriea_match_players (
    ...
    with engine.connect() as conn:
        conn.execute(query)
        conn.commit()
    logging.info("✅ Database Serie A inizializzato (Metodo Partita per Partita).")

# ─── MOTORE DI ESTRAZIONE SICURO ───────────────────────────────────────
    ...
    while retries < 5:
        try:
            ...
            if e.status == 429:
                ...
                logging.error(f"❌ Errore HTTP {e.status}. Riprovo tra 10s...")
                retries += 1
                await asyncio.sleep(10)
        except Exception as e:
            logging.error(f"❌ Errore connessione: {e}. Riprovo...")
            retries += 1
            await asyncio.sleep(10)
    return None
...
async def process_season(session, season):
    ...
    logging.info(f"\n━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━")
    logging.info(f"🇮🇹 INIZIO STAGIONE SERIE A: {season}")
    
    # 1. Recupero elenco partite della stagione
    matches = await safe_fetch(understat.get_league_results, LEAGUE, season)
    if not matches:
        logging.error(f"⚠️ Impossibile recuperare il calendario per la stagione {season}")
        return

    played_matches = [m for m in matches if m.get("isResult")]
    logging.info(f"🔍 Trovate {len(played_matches)} partite giocate. Inizio scansione profonda...")
    ...
    with engine.connect() as conn:
        for idx, m in enumerate(played_matches, 1):
            ...
            # 2. ENTRA NELLA SINGOLA PARTITA (Scansione molecolare)
            ...
            if match_players:
                ...
                for side in ['h', 'a']:
                    ...
                    for p_id, p_data in match_players.get(side, {}).items():
                        query = text("""
                            INSERT INTO seriea_match_players 
...
async def main():
    initialize_database()
    # Connettore corazzato per evitare chiusure improvvise
    ...
    logging.info("\n🎯 MISSIONE COMPIUTA: Serie A scaricata riga per riga.")
