"""
🚀 ESTRATTORE BUNDESLIGA - METODO "PARTITA PER PARTITA"
Struttura identica a Premier, Serie A e La Liga.
Lento, chirurgico, progettato per non essere rilevato.
"""
import asyncio
import aiohttp
import random
...
async def apply_jitter():
    # Fondamentale per simulare il comportamento di un utente reale
...
# ─── DATABASE (Tabella Bundesliga) ────────────────────────────────────
def initialize_database():
    query = text("""
        CREATE TABLE IF NOT EXISTS bundesliga_match_players (
    ...
    with engine.connect() as conn:
        conn.execute(query)
        conn.commit()
    logging.info("✅ Database Bundesliga inizializzato correttamente.")
...
async def process_season(session, season):
    ...
    logging.info(f"\n━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━")
    logging.info(f"🇩🇪 INIZIO STAGIONE BUNDESLIGA: {season}")
    
    # 1. Recupero calendario della stagione
    matches = await safe_fetch(understat.get_league_results, LEAGUE, season)
    if not matches:
        logging.error(f"⚠️ Impossibile recuperare partite per la Bundesliga stagione {season}")
        return
    ...
    with engine.connect() as conn:
        for idx, m in enumerate(played_matches, 1):
            ...
            # 2. Scansione molecolare del match
...
async def main():
    initialize_database()
    # Connettore forzato per stabilità a lungo termine
    ...
    logging.info("\n🎯 BUNDESLIGA COMPLETATA. TUTTE LE LEGHE PRINCIPALI SONO IN ARCHIVIO.")
