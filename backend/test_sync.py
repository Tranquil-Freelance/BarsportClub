...
DB_CONFIG = {
    "user": "postgres",
    "pass": "tua_password",  # <--- INSERISCI LA TUA PASSWORD
...
class UnderstatJSExecutor:
    ...
    def _trigger_hibernation(self):
        logging.warning(f"Ibernazione di {HIBERNATION_TIME/60} minuti avviata.")
    ...
    def get_match_ids(self, league, season):
        ...
        for _ in range(45):
            if "Just a moment..." in self.page.html or "cf-browser-verification" in self.page.html:
                logging.warning("Cloudflare in azione sulla pagina Lega. Clicca il Captcha se necessario...")
                time.sleep(3)
                continue
    ...
    def extract_with_js(self, match_id):
        ...
        for _ in range(60):
            if "Just a moment..." in self.page.html or "cf-browser-verification" in self.page.html:
                logging.warning(f"Cloudflare bloccante su Match {match_id}. Risolvi il Captcha...")
                time.sleep(3)
                continue
    ...
    def inject_data(self, match_id, data):
        ...
        # ON CONFLICT DO NOTHING (senza specificare l'ID) assorbe qualsiasi violazione di unicità!
        if m_map:
